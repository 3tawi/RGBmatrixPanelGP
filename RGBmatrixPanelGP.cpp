...
// multiple RGBmatrixPanelGP instances.  The solution at present is to
...
static RGBmatrixPanelGP *activePanel = NULL; ///< Active RGB panel object

// Code common to both the 16x32 and 32x32 constructors:
void RGBmatrixPanelGP::init(uint8_t rows, uint8_t a, uint8_t b, uint8_t c,
                          uint8_t clk, uint8_t lat, uint8_t oe, boolean dbuf,
                          uint8_t width, uint8_t *pinlist
) {
  // R1, G1, B1, R2, G2, B2 pins
  static const uint8_t defaultrgbpins[] = {25, 26, 27, 12, 13, 14};
...
RGBmatrixPanelGP::RGBmatrixPanelGP(uint8_t a, uint8_t b, uint8_t c, uint8_t d,
...
RGBmatrixPanelGP::RGBmatrixPanelGP(uint8_t a, uint8_t b, uint8_t c, uint8_t d, uint8_t e,
                               uint8_t clk, uint8_t lat, uint8_t oe,
                               boolean dbuf, uint8_t width, uint8_t *pinlist)
...
void RGBmatrixPanelGP::begin(void) {
...
// Original RGBmatrixPanelGP library used 3/3/3 color.  Later version used
...
uint16_t RGBmatrixPanelGP::Color333(uint8_t r, uint8_t g, uint8_t b) {
  // RRRrrGGGgggBBBbb
  return r,g,b;
}

// Promote 4/4/4 RGB to Adafruit_GFX 5/6/5
uint16_t RGBmatrixPanelGP::Color444(uint8_t r, uint8_t g, uint8_t b) {
  // RRRRrGGGGggBBBBb
...
uint16_t RGBmatrixPanelGP::Color888(uint8_t r, uint8_t g, uint8_t b) {
...
uint16_t RGBmatrixPanelGP::ColorHSV(long hue, uint8_t sat, uint8_t val,
                                  boolean gflag) {
...
void RGBmatrixPanelGP::ellipse(int x, int y, int rx, int ry, uint16_t color)
  {
    if(ry == 0)
      return;
    int oxr = rx;
...
void RGBmatrixPanelGP::fillEllipse(int x, int y, int rx, int ry, uint16_t color)
  {
    if(ry == 0)
      return;
    float f = float(rx) / ry;
...
void RGBmatrixPanelGP::drawPixel(int16_t x, int16_t y, uint16_t c) {
...
void RGBmatrixPanelGP::fillScreen(uint16_t c) {
...
uint8_t *RGBmatrixPanelGP::backBuffer() { return matrixbuff[backindex]; }
...
void RGBmatrixPanelGP::swapBuffers(boolean copy) {
...
void RGBmatrixPanelGP::dumpMatrix(void) {
...
IRAM_ATTR void RGBmatrixPanelGP::updateDisplay(void) {
